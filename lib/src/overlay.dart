...
///
///
OverlayNotificationEntry showOverlay(
  ...
  final overlayNotification = findOverlayState(context: context);
  final overlay = overlayNotification?.overlayState;
  if (overlayNotification == null || overlay == null) {
    assert(() {
  ...
  final oldSupportEntry = overlayNotification.getEntry(key: overlayKey);
  ...
  // If we got a showing OverlayNotification with [key], we should dismiss it before showing a new.
  ...
  final supportEntry = OverlayNotificationEntry._internal(
      entry, overlayKey, stateKey, overlayNotification);
  overlayNotification.addEntry(supportEntry, key: overlayKey);
